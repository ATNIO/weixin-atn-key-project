...
var _UTF = __webpack_require__(97);

var _UTF2 = _interopRequireDefault(_UTF);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var bigInt = __webpack_require__(101);
...
function split_ints(secrets) {
  ...
  working = _UTF2.default.setBytesFromString(secrets);
...
function merge_ints(secrets) {
  ...
  return _UTF2.default.getStringFromBytes(bytes);
}
...
function create(minimum, shares, raw) {
  ...
  for (i = 0; i < shares; i++) {
    for (j in secret) {
      ...
      if ((0, _typeof3.default)(result[i]) !== (0, _typeof3.default)("string")) {
        result[i] = "";
      }
...
function QR8bitByte(t) {
  ...
  this.typeNumber = t, this.errorCorrectLevel = e, this.modules = null, this.moduleCount = 0, this.dataCache = null, this.dataList = new Array();
}QR8bitByte.prototype = { getLength: function getLength(t) {
    return this.data.length;
  }, write: function write(t) {
    for (var e = 0; e < this.data.length; e++) {
      t.put(this.data.charCodeAt(e), 8);
    }
  } }, QRCode.prototype = { addData: function addData(t) {
    var e = new QR8bitByte(t);this.dataList.push(e), this.dataCache = null;
    ...
    if (this.typeNumber < 1) {
      var t = 1;for (t = 1; t < 40; t++) {
        for (var e = QRRSBlock.getRSBlocks(t, this.errorCorrectLevel), r = new QRBitBuffer(), o = 0, n = 0; n < e.length; n++) {
          ...
          var i = this.dataList[n];r.put(i.mode, 4), r.put(i.getLength(), QRUtil.getLengthInBits(i.mode, t)), i.write(r);
    ...
    this.moduleCount = 4 * this.typeNumber + 17, this.modules = new Array(this.moduleCount);for (var r = 0; r < this.moduleCount; r++) {
      this.modules[r] = new Array(this.moduleCount);for (var o = 0; o < this.moduleCount; o++) {
    ...
    for (var t = 0, e = 0, r = 0; r < 8; r++) {
      this.makeImpl(!0, r);var o = QRUtil.getLostPoint(this);(0 == r || t > o) && (t = o, e = r);
    }return e;
  }, createMovieClip: function createMovieClip(t, e, r) {
    var o = t.createEmptyMovieClip(e, r);this.make();for (var n = 0; n < this.modules.length; n++) {
      for (var i = 1 * n, a = 0; a < this.modules[n].length; a++) {
        var s = 1 * a;this.modules[n][a] && (o.beginFill(0, 100), o.moveTo(s, i), o.lineTo(s + 1, i), o.lineTo(s + 1, i + 1), o.lineTo(s, i + 1), o.endFill());
      }
    }return o;
    ...
    for (var t = QRUtil.getPatternPosition(this.typeNumber), e = 0; e < t.length; e++) {
      for (var r = 0; r < t.length; r++) {
        var o = t[e],
            n = t[r];if (null == this.modules[o][n]) for (var i = -2; i <= 2; i++) {
          for (var a = -2; a <= 2; a++) {
    ...
    for (var r = this.errorCorrectLevel << 3 | e, o = QRUtil.getBCHTypeInfo(r), n = 0; n < 15; n++) {
      ...
      i = !t && 1 == (o >> n & 1);n < 8 ? this.modules[8][this.moduleCount - n - 1] = i : n < 9 ? this.modules[8][15 - n - 1 + 1] = i : this.modules[8][15 - n - 1] = i;
      ...
      for (6 == a && a--;;) {
        for (var s = 0; s < 2; s++) {
          if (null == this.modules[o][a - s]) {
            var u = !1;i < t.length && (u = 1 == (t[i] >>> n & 1)), QRUtil.getMask(e, o, a - s) && (u = !u), this.modules[o][a - s] = u, -1 == --n && (i++, n = 7);
          }
  ...
  for (var o = QRRSBlock.getRSBlocks(t, e), n = new QRBitBuffer(), i = 0; i < r.length; i++) {
    var a = r[i];n.put(a.mode, 4), n.put(a.getLength(), QRUtil.getLengthInBits(a.mode, t)), a.write(n);
  }var s = 0;for (i = 0; i < o.length; i++) {
    s += o[i].dataCount;
  }if (n.getLengthInBits() > 8 * s) throw new Error("code length overflow. (" + n.getLengthInBits() + ">" + 8 * s + ")");for (n.getLengthInBits() + 4 <= 8 * s && n.put(0, 4); n.getLengthInBits() % 8 != 0;) {
    n.putBit(!1);
  ...
  for (var r = 0, o = 0, n = 0, i = new Array(e.length), a = new Array(e.length), s = 0; s < e.length; s++) {
    var u = e[s].dataCount,
        h = e[s].totalCount - u;o = Math.max(o, u), n = Math.max(n, h), i[s] = new Array(u);for (var l = 0; l < i[s].length; l++) {
      i[s][l] = 255 & t.buffer[l + r];
    }r += u;var f = QRUtil.getErrorCorrectPolynomial(h),
        g = new QRPolynomial(i[s], f.getLength() - 1).mod(f);a[s] = new Array(f.getLength() - 1);for (l = 0; l < a[s].length; l++) {
      var R = l + g.getLength() - a[s].length;a[s][l] = R >= 0 ? g.get(R) : 0;
    }
  }var d = 0;for (l = 0; l < e.length; l++) {
    d += e[l].totalCount;
  }var c = new Array(d),
      Q = 0;for (l = 0; l < o; l++) {
    ...
    switch (t) {case QRMaskPattern.PATTERN000:
        return (e + r) % 2 == 0;case QRMaskPattern.PATTERN001:
        return e % 2 == 0;case QRMaskPattern.PATTERN010:
        return r % 3 == 0;case QRMaskPattern.PATTERN011:
        return (e + r) % 3 == 0;case QRMaskPattern.PATTERN100:
        return (Math.floor(e / 2) + Math.floor(r / 3)) % 2 == 0;case QRMaskPattern.PATTERN101:
        return e * r % 2 + e * r % 3 == 0;case QRMaskPattern.PATTERN110:
        return (e * r % 2 + e * r % 3) % 2 == 0;case QRMaskPattern.PATTERN111:
    ...
    if (1 <= e && e < 10) switch (t) {case QRMode.MODE_NUMBER:
        return 10;case QRMode.MODE_ALPHA_NUM:
        return 9;case QRMode.MODE_8BIT_BYTE:case QRMode.MODE_KANJI:
        return 8;default:
        throw new Error("mode:" + t);} else if (e < 27) switch (t) {case QRMode.MODE_NUMBER:
      ...
      if (!(e < 41)) throw new Error("type:" + e);switch (t) {case QRMode.MODE_NUMBER:
    ...
    }var l = 0;for (n = 0; n < e; n++) {
      for (o = 0; o < e; o++) {
        t.isDark(o, n) && l++;
      }
    }return r += 10 * (Math.abs(100 * l / e / e - 50) / 5);
  } }, QRMath = { glog: function glog(t) {
    if (t < 1) throw new Error("glog(" + t + ")");return QRMath.LOG_TABLE[t];
  ...
  }, EXP_TABLE: new Array(256), LOG_TABLE: new Array(256) }, i = 0; i < 8; i++) {
  ...
  var e, r, o, n;for (e = "", o = t.length, r = 0; r < o; r++) {
  ...
  t = t || {}, (t = (0, _assign2.default)({ width: 256, height: 256, typeNumber: -1, correctLevel: QRErrorCorrectLevel.H, background: "#ffffff", foreground: "#000000" }, t)).canvasId ? function () {
    var e = new QRCode(t.typeNumber, t.correctLevel);e.addData(utf16to8(t.text)), e.make();for (var r = t._this ? wx.createCanvasContext && wx.createCanvasContext(t.canvasId, t._this) : wx.createCanvasContext && wx.createCanvasContext(t.canvasId), o = t.width / e.getModuleCount(), n = t.height / e.getModuleCount(), i = 0; i < e.getModuleCount(); i++) {
      for (var a = 0; a < e.getModuleCount(); a++) {
        var s = e.isDark(i, a) ? t.foreground : t.background;r.setFillStyle(s);var u = Math.ceil((a + 1) * o) - Math.floor(a * o),
            h = Math.ceil((i + 1) * o) - Math.floor(i * o);r.fillRect(Math.round(a * o), Math.round(i * n), u, h);
      }
    }r.draw(!1, function (e) {
      t.callback && t.callback(e);
    });
  }() : console.warn("please you set canvasId!");
}QRPolynomial.prototype = { get: function get(t) {
    return this.num[t];
    ...
    if (this.getLength() - t.getLength() < 0) return this;for (var e = QRMath.glog(this.get(0)) - QRMath.glog(t.get(0)), r = new Array(this.getLength()), o = 0; o < this.getLength(); o++) {
      r[o] = this.get(o);
  ...
  var r = QRRSBlock.getRsBlockTable(t, e);if (void 0 == r) throw new Error("bad rs block @ typeNumber:" + t + "/errorCorrectLevel:" + e);for (var o = r.length / 3, n = new Array(), i = 0; i < o; i++) {
    for (var a = r[3 * i + 0], s = r[3 * i + 1], u = r[3 * i + 2], h = 0; h < a; h++) {
      n.push(new QRRSBlock(s, u));
    }
  }return n;
}, QRRSBlock.getRsBlockTable = function (t, e) {
  switch (e) {case QRErrorCorrectLevel.L:
      return QRRSBlock.RS_BLOCK_TABLE[4 * (t - 1) + 0];case QRErrorCorrectLevel.M:
      return QRRSBlock.RS_BLOCK_TABLE[4 * (t - 1) + 1];case QRErrorCorrectLevel.Q:
      return QRRSBlock.RS_BLOCK_TABLE[4 * (t - 1) + 2];case QRErrorCorrectLevel.H:
      return QRRSBlock.RS_BLOCK_TABLE[4 * (t - 1) + 3];default:
      return;}
}, QRBitBuffer.prototype = { get: function get(t) {
...
var _fromCodePoint = __webpack_require__(98);
...
var UTF8 = {
  ...
  'isNotUTF8': function isNotUTF8(bytes, byteOffset, byteLength) {
    try {
  ...
  'getCharLength': function getCharLength(theByte) {
  ...
  'getCharCode': function getCharCode(bytes, byteOffset, charLength) {
    var charCode = 0,
        mask = '';
  ...
  'getStringFromBytes': function getStringFromBytes(bytes, byteOffset, byteLength, strict) {
    var charLength,
        chars = [];
    byteOffset = byteOffset | 0;
    ...
    for (; byteOffset < byteLength; byteOffset++) {
      ...
      if (byteOffset + charLength > byteLength) {
        ...
        chars.push((0, _fromCodePoint2.default)(UTF8.getCharCode(bytes, byteOffset, charLength, strict)));
      }
  ...
  'getBytesForCharCode': function getBytesForCharCode(charCode) {
    if (charCode < 128) {
      return 1;
  ...
  'setBytesFromCharCode': function setBytesFromCharCode(charCode, bytes, byteOffset, neededBytes) {
    charCode = charCode | 0;
    bytes = bytes || [];
    byteOffset = byteOffset | 0;
  ...
  'setBytesFromString': function setBytesFromString(string, bytes, byteOffset, byteLength, strict) {
    string = string || '';
    bytes = bytes || [];
    byteOffset = byteOffset | 0;
